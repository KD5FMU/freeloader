...
if (isset($_SESSION['freeloader_loggedin']) && isset($_SESSION['last_activity'])) {
    if (time() - $_SESSION['last_activity'] > $TIMEOUT) {
        session_unset();
        session_destroy();
        session_start(); // start fresh after destroy
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'])) {
    if (isset($FREELoader_PASSWORD) && $_POST['password'] === $FREELoader_PASSWORD) {
    ...
    <div class="container">
        <?php if (!$logged_in): ?>
            ...
            <form method="post" style="max-width:400px;margin:0 auto;">
                <input type="password" name="password" placeholder="Enter Password" style="width:100%;padding:12px;margin:10px 0; box-sizing:border-box;" required autofocus>
